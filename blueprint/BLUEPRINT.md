...
>
> **This file is the single source of truth.** It is kept in its own repository, separate from
> the app code. Every time a feature is added or changed in the app, this file is updated to
> match — automatically, without being asked.
>
> **Last synced with codebase:** 2026-06-08 (commit `75438ed`)
...
### `src/app/globals.css`
- `@import "tailwindcss";`
- Defines `:root` light tokens and a `@media (prefers-color-scheme: dark)` block with full
...
│   ├── question/
│   │   ├── question-card.tsx               # Public flashcard (answer flow)
│   │   └── question-option.tsx             # Single option button
│   └── ui/
...
- `<html lang="en" className="...variables h-full antialiased">`, `<body className="min-h-full flex flex-col">`.
- `metadata`: title "Create Next App", description "Generated by create next app" *(default — not yet customized)*.
...
- Renders header ("Entrance Question" + non-functional 🌙 dark-mode and "👤 Sign in" buttons),
  `<TopFilterBar>`, and a centered `<QuestionCard>` fed the **filtered** published questions.
...
- **Visual language:** rounded-2xl/3xl cards, `border` + `bg-white` / `dark:bg-slate-900`,
  `shadow-sm`, `font-black`/`font-bold` headings, pill badges. Full dark-mode classes everywhere
  (`dark:` variants), driven by `prefers-color-scheme` (no manual toggle wired up yet).
...
- **2026-06-08** — Blueprint repository created; full spec captured from commit `75438ed`.
